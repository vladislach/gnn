...
def train_and_validate(model, device, train_loader, val_loader, loss_fn, metric, num_epochs, optimizer, scheduler=None, verbose=True):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for batch in data_loader:
            encoded_atoms, edges, natoms, dGsolv = batch
            encoded_atoms, edges, dGsolv = encoded_atoms.to(device), edges.to(device), dGsolv.to(device)
            
            optimizer.zero_grad()
            dGsolv_pred = model(encoded_atoms, edges, natoms)
